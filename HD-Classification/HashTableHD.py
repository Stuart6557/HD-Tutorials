...
"""
def is_match(hash_table_hv, enc_hv):
	cos_sim = dot(hash_table_hv, enc_hv) / (norm(hash_table_hv) * norm(enc_hv))
	print(cos_sim)
	return cos_sim > 0.9
...
def train(X_train, X_test, y_test, D=500, alg='rp'):
	...
	if alg in ['rp', 'rp-sign']:
		...
		print('Encoding ' + str(len(X_train)) + ' train data')
		...
		for i in range(len(test_enc_hvs)):
			predict = is_match(hash_table_hv, test_enc_hvs[i])
			if predict == y_test[i]:
				correct += 1
